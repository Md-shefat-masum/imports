			...
			<div class="card-header header-part">
				<div class="row">
					...
					<div class="col-md-6 text-right card_header_btn">
						<a href="{{ route('addSlider') }}" class="btn"><i class="fa fa-plus-circle"></i> Add
							Slider</a>
					...
					<tbody>
						...
						<tr>
							...
							<td style="display: block ruby;">
								<form class="form-inline" action="{{ route('editSlider') }}" method="post"
									...
									<div class="form-group">
										<select class="form-control" id="sel1" name="status" required>
											<option value="1" {{ ($slider->slider_position == 1)? 'selected' : '' }}>Up
											</option>
											<option value="0" {{ ($slider->slider_position == 0)? 'selected' : '' }}>
												Down</option>
										</select>
									</div>
									<button type="submit" class="btn btn-primary">Update</button>
								</form>
								<form method="POST" action="{{ route('sliderDelete', $slider->id) }}"
