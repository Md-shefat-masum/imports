			...
			<div class="card-header header-part">
				<div class="row">
					<div class="col-md-6 card_header_title">
						<h3><i class="fa fa-gg-circle"></i> All HotSale</h3>
					</div>
								...
								<form class="form-inline" action="{{ route('editHotSale') }}" method="post"
									...
									<div class="form-group">
										<select class="form-control" id="sel1" name="status" required>
											<option value="1" {{ ($slider->hot_sale_status == 1)? 'selected' : '' }}>Up
											</option>
											<option value="0" {{ ($slider->hot_sale_status == 0)? 'selected' : '' }}>
												Down</option>
										</select>
									</div>
									<button type="submit" class="btn btn-primary">Update</button>
								</form>
								<form method="POST" action="{{ route('hot_sale_Delete', $slider->id) }}"
