			...
			<div class="card-header header-part">
				<div class="row">
					...
					<div class="col-md-6 text-right card_header_btn">
						<a href="{{ route('homeProduct') }}" class="btn"><i class="fa fa-reply"
								aria-hidden="true"></i>
								Back To Home Product</a>
				...
				<table cellspacing="0" bordercolor="gray" id="allTable"
					...
					<tbody>
						...
						@forelse($product as $sp)
							...
							@php
								$sliderPro = DB::table('front_page_product')->where('product_id', $sp->id)->get();
							@endphp
							@if (count($sliderPro)<1)
								...
								<tr>
								  ...
								  @endphp
									...
									<td>
										<form class="form-inline" action="{{ route('addToHome') }}" method="post" style="display: inline-block;">
											@csrf
											<input type="hidden" name="pro_id" value="{{$sp->id}}">
											<button type="submit" class="btn btn-info">Make Home</button>
										</form>
									</td>
							...
							@endif
						@empty
							<h3>No Product Found Or This Product is already Home</h3>
						@endforelse
